...
folder_name="$(basename "$(dirname "$ZED_FILE")")"
dest_dir="/home/abuturab/Documents/git/w3/w3-wiki/static/notes/${folder_name}"
mkdir -p "$dest_dir"
# Use md filename as base, override via argument
...
markdown_link="![](/notes/${folder_name}/${file_name})"
insert_markdown() {
  local target_file="$1"
  local text="$2"
  local row="${3:-}"
